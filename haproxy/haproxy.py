...
class Haproxy(object):
    ...
    # class var
    container_uri = os.getenv("TUTUM_CONTAINER_API_URI")
    service_uri = os.getenv("TUTUM_SERVICE_API_URI")
    ...
    def __init__(self):
        self.ssl = None
        if self.container_uri and self.service_uri and self.tutum_auth:
            logger.info("Load HAProxy definition through REST Api")
            container = self.fetch_tutum_obj(self.container_uri)
            service = self.fetch_tutum_obj(self.service_uri)
            self.linked_services = [srv.get("to_service") for srv in service.linked_to_service]
            self.link_specs = Specs(container, service)
        else:
            logger.info("Load HAProxy definition from environment variables")
            self.linked_services = None
            self.link_specs = Specs()
    ...
    def update(self):
        ...
        if self.service_uri and self.container_uri and self.tutum_auth:
            if self.cfg != cfg:
                if not self.cfg:
    ...
    def _run(self):
        if self.haproxy_process:
            ...
            process = subprocess.Popen(self.const_command + ["-sf", str(self.haproxy_process.pid)])
            ...
            # Launch haproxy
            logger.info("Launching HAProxy\n******************************")
            return subprocess.Popen(self.const_command)
