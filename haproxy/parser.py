...
class Specs(object):
    ...
    def _parser_service_aliases(self, tutum_haproxy_services):
        if tutum_haproxy_services:
            service_aliases = [service["name"].upper() for service in tutum_haproxy_services.linked_to_service]
        else:
            service_aliases = []
...
class RouteParser(object):
    ...
    @staticmethod
    def parse(specs, service_aliases, tutum_haproxy_container=None):
        if tutum_haproxy_container:
